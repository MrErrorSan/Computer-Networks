...
void *ConnectionRefuser(void *message)
{
    ...
    printf("Client Connection with IP: %s and Port No is refused: %i\n", inet_ntoa(client_addr.sin_addr), ntohs(client_sock));
...
int main()
{
    ...
    while (1)
    {
        ...
        if (ActiveClients < 3)
        {
            ...
            ActiveClients++;
            pthread_create(&threads[thread_index], NULL, ThreadsFunction, (void *)&Clientsockets[thread_index]);
            thread_index++;
        }
        else
        {
